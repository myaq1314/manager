...
public class IOUtil {
    ...
    // 将结果集转换为 json 时，把 null 转换为 Date 类型会报错。这时输出一个空字符串
    ...
    /**  格式化 金额的单位 为 万元 */
    public static void formatMoney(List<Object> list, String key) {
        if (list == null) {
            return;
        }
    ...
    /**  格式化 金额的单位 为 万元 */
    public static void formatMoney(Map<String, Object> map) {
        if (map == null) {
            return;
        }
        ...
        for (Iterator<String> it = set.iterator(); it.hasNext(); ) {
            ...
            if (!Util.isNullOrEmptyOrZero(value)) {
                map.put(key, value.divide(new BigDecimal(10000)));
            }
        }
    }
